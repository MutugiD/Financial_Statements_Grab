...
def Main_Expenses(df):
    ...
    expenses = Expenses[(Expenses['Unnamed: 3'] <= 5000) & (Expenses['Unnamed: 3'] >= -5000)&(Expenses['Unnamed: 4'] <= 5) & (Expenses['Unnamed: 4'] >= -5)]. rename(columns=  {'Profit & Loss': 'Ledger', 'Unnamed: 1': 'Oct 2020','Unnamed: 2': 'Nov 2020','Unnamed: 3': 'Variance ($)', 'Unnamed: 4': 'Variance (%)' }).fillna('')
    expenses.set_index("Ledger", inplace =True)
    main_expenses= expenses.append(expenses[['Oct 2020', 'Nov 2020','Variance ($)','Variance (%)']].sum().rename('Subtotal Main Expenses'))
...
def Other_Expenses(df):
    ...
    other_expenses= OtherExpenses[(OtherExpenses['Unnamed: 3'] <= 5000) & (OtherExpenses['Unnamed: 3'] >= -5000)&(OtherExpenses['Unnamed: 4'] <= 5) & (OtherExpenses['Unnamed: 4'] >= -5)].rename(columns=  {'Profit & Loss': 'Ledger', 'Unnamed: 1': 'Oct 2020','Unnamed: 2': 'Nov 2020','Unnamed: 3': 'Variance ($)', 'Unnamed: 4': 'Variance (%)' })
    other_expenses = other_expenses.append(other_expenses[['Oct 2020', 'Nov 2020','Variance ($)','Variance (%)']].sum().rename('All Other Expenses'))
...
def Net_Income(df): 
    ...
    net_income = NetIncome.rename(columns= {'Unnamed: 1': 'Oct 2020','Unnamed: 2': 'Nov 2020','Unnamed: 3': 'Variance ($)', 'Unnamed: 4': 'Variance (%)' }).fillna('')
    return net_income
...
def main(): 
    ...
    writer = pd.ExcelWriter(r'E:\Python Projects\Grindstone\Variance Report Data - Output Example\Example Input Data\Grind_Financials.xlsx', engine='xlsxwriter')
    for idx in range(len(data)):
        df = data[idx]
        filename = df['Profit & Loss'].iloc[0]
        print('Accessing df {}'.format(idx))
        sheet = str(filename)
        df =df.iloc[1:,]
        ...
        main_file.to_excel(writer, sheet_name = sheet)
